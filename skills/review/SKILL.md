...
When the review skill is invoked and **no session exists**, automatically launch Hunk:

1. Detect the base branch — check for `main`, then `master` (whichever exists in the repo)
2. Launch Hunk in a new terminal window:
   - **macOS/Ghostty**: `open -na Ghostty.app --args --command="hunk diff <base>...HEAD"`
   - **Linux/other**: `hunk diff <base>...HEAD &` (or use the user's terminal emulator with `-e` equivalent)
3. Wait ~2 seconds for the session to initialize, then proceed with the workflow below

If a session already exists, reuse it — ask the user whether to reload or keep the current diff.
